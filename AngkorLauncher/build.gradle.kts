...
val getBuildType = {
    val buildType = System.getenv("ANGKOR_BUILD_TYPE") ?: "DEBUG"
    logger.warn("BUILD: Build Type --> $buildType")
    buildType
}

val nameId = "com.movtery.angkorlauncher"
val appId = "com.angkorlauncher.khmer"
val generatedAngkorDir = file("$buildDir/generated/source/angkor/java")
...
android {
    ...
    sourceSets["main"].java.srcDirs(generatedAngkorDir)
...
tasks.register("generateInfoDistributor") {
    doLast {
        ...
        generateJavaClass(generatedAngkorDir, "com.movtery.angkorlauncher", "InfoDistributor", constantMap)
    }
}
