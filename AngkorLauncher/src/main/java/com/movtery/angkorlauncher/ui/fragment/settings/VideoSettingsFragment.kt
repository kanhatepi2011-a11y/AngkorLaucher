...
import com.movtery.angkorlauncher.databinding.SettingsFragmentVideoBinding
import com.movtery.angkorlauncher.event.single.LauncherIgnoreNotchEvent
...
import com.movtery.angkorlauncher.ui.dialog.LocalRendererPluginDialog
import com.movtery.angkorlauncher.ui.dialog.TipDialog
import com.movtery.angkorlauncher.ui.fragment.settings.wrapper.BaseSettingsWrapper
import com.movtery.angkorlauncher.ui.fragment.settings.wrapper.ListSettingsWrapper
import com.movtery.angkorlauncher.ui.fragment.settings.wrapper.SeekBarSettingsWrapper
import com.movtery.angkorlauncher.ui.fragment.settings.wrapper.SwitchSettingsWrapper
...
class VideoSettingsFragment : AbstractSettingsFragment(R.layout.settings_fragment_video, SettingCategory.VIDEO) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.rendererDownload.setOnClickListener {
            if (allowRendererAction()) ZHTools.openLink(context, UrlManager.URL_FCL_RENDERER_PLUGIN)
        }
        ...
        binding.driverDownload.setOnClickListener {
            if (allowRendererAction()) ZHTools.openLink(context, UrlManager.URL_FCL_DRIVER_PLUGIN)
        }

        ZHTools.setTooltipText(binding.rendererDownload, getString(R.string.setting_category_download))
        ZHTools.setTooltipText(binding.rendererLocalImportManage, getString(R.string.setting_renderer_local_manage))
        ZHTools.setTooltipText(binding.driverDownload, getString(R.string.setting_category_download))
    ...
    private var lastRendererActionTime = 0L

    private fun allowRendererAction(): Boolean {
        val now = android.os.SystemClock.elapsedRealtime()
        if (now - lastRendererActionTime < RENDERER_ACTION_DEBOUNCE_MS) return false
        lastRendererActionTime = now
        return true
    }
    ...
    companion object {
        private const val RENDERER_ACTION_DEBOUNCE_MS = 600L
