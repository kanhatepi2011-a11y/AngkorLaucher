...
import com.movtery.angkorlauncher.feature.version.VersionsManager.getAngkorVersionPath
import com.movtery.angkorlauncher.setting.AllSettings
import com.movtery.angkorlauncher.utils.stringutils.StringUtils.getStringNotNull
...
class VersionConfig(private var versionPath: File) : Parcelable {
    ...
    fun saveWithThrowable() {
        ...
        val angkorVersionPath = getAngkorVersionPath(versionPath)
        val configFile = File(angkorVersionPath, "VersionConfig.json")
        if (!angkorVersionPath.exists()) angkorVersionPath.mkdirs()
    ...
    companion object CREATOR : Parcelable.Creator<VersionConfig> {
        ...
        fun parseConfig(versionPath: File): VersionConfig {
            ...
            val oldConfigFile = File(getAngkorVersionPath(versionPath), "AngkorVersion.cfg")
            val configFile = File(getAngkorVersionPath(versionPath), "VersionConfig.json")
