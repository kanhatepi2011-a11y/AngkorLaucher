...
import com.movtery.angkorlauncher.event.single.RefreshVersionsEvent.MODE.END
import com.movtery.angkorlauncher.event.single.RefreshVersionsEvent.MODE.START
...
import com.movtery.angkorlauncher.feature.version.utils.VersionInfoUtils
...
import com.movtery.angkorlauncher.utils.stringutils.SortStrings
...
object VersionsManager {
     ...
     * @see com.movtery.angkorlauncher.event.single.RefreshVersionsEvent
     */
    ...
    private fun processVersionFile(versionsHome: String, versionFile: File, refreshVersionInfo: Boolean) {
        if (versionFile.exists() && versionFile.isDirectory) {
            ...
            if (jsonFile.exists() && jsonFile.isFile) {
                isVersion = true
                val versionInfoFile = File(getAngkorVersionPath(versionFile), "VersionInfo.json")
    ...
    /**
     * @return 获取 Angkor 启动器版本标识文件夹
     */
    fun getAngkorVersionPath(version: Version) = File(version.getVersionPath(), InfoDistributor.LAUNCHER_NAME)

    /**
     * @return 通过目录获取 Angkor 启动器版本标识文件夹
     */
    fun getAngkorVersionPath(folder: File) = File(folder, InfoDistributor.LAUNCHER_NAME)

    /**
     * @return 通过名称获取 Angkor 启动器版本标识文件夹
     */
    fun getAngkorVersionPath(name: String) = File(getVersionPath(name), InfoDistributor.LAUNCHER_NAME)

    /**
     * @return 获取当前版本设置的图标
     */
    fun getVersionIconFile(version: Version) = File(getAngkorVersionPath(version), "VersionIcon.png")

    /**
     * @return 通过名称获取当前版本设置的图标
     */
    fun getVersionIconFile(name: String) = File(getAngkorVersionPath(name), "VersionIcon.png")
